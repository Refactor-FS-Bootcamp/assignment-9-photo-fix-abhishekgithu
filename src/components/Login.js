...
const Login = () => {
    ...
    const loginToApp = e => {
        ...
        auth.signInWithEmailAndPassword(email, password)
            .then(userAuth => {
                dispatch(login({ email: userAuth.user.email, displayName: userAuth.user.displayName, photoUrl: userAuth.user.photoURL }))
            })
    }
    ...
    return (
        <div className="login">
            ...
            <p>Not a member?{' '}
                <span onClick={register} className="login__register">RegisterNow</span>
            </p>
        </div>
    )
}
